...
export default function Scores() {
  ...
  return (
    <div>
      <h1 className="font-serif font-black text-2xl text-green-900 mb-4">⭐ ให้คะแนนนักเรียน</h1>
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={s.id} className="border-t border-green-50 hover:bg-green-50/50">
                  ...
                  <td className="px-4 py-3 font-serif font-black text-green-700 text-xl">{s.score.toLocaleString()}</td>
                  <td className="px-4 py-3">
      ...
      >
        <div className="mb-3">
          ...
          <input type="number" min={1} className="input text-center text-2xl font-serif font-black" value={scoreForm.amount} onChange={(e) => setScoreForm({ ...scoreForm, amount: e.target.value })} />
        </div>
        <div className="mb-3">
        ...
        <div className="max-h-80 overflow-y-auto space-y-2">
          {!history.length ? <Empty icon="📭" text="ยังไม่มีประวัติ" /> : history.map((h) => (
            <div key={h.id} className="flex items-center gap-3 bg-green-50 rounded-xl px-4 py-2.5">
              <div className={`font-serif font-black text-lg min-w-[50px] text-center ${h.delta > 0 ? "text-green-700" : "text-red-600"}`}>
                {h.delta > 0 ? "+" : ""}{h.delta}
              </div>
